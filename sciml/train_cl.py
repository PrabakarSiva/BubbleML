...
from op_lib.push_vel_trainer_cl import PushVelTrainerCl
...
def build_datasets(cfg):
    ...
    train_datasets = []
    for dataset in cfg.dataset.train_paths:
        train_datasets.append(HDF5ConcatDataset([
                DatasetClass[0](dataset,
                                steady_time=cfg.dataset.steady_time,
    ...
    max_temps = []
    max_vel = []
    for tr_dataset in train_datasets: 
        max_temps.append(tr_dataset.normalize_temp_())
        max_vel.append(tr_dataset.normalize_vel_())

    print (max_temps)
    print (max_vel)
    train_max_temp = max(max_temps)
    train_max_vel = max(max_vel)    
    print(train_max_temp)
    print(train_max_vel)
    ...
    for tr_dset in train_datasets:
        print(tr_dset[0][0].size())
    
    return train_datasets, val_dataset, train_max_temp, train_max_vel

def build_train_dataloader(train_dataset, cfg):
...
def build_val_dataloader(val_dataset, cfg):
    if cfg.experiment.distributed:
...
def train_app(cfg):
    ...
    train_datasets, val_dataset, train_max_temp, train_max_vel = build_datasets(cfg)
    
    train_dataloaders = []
    train_size = 0
    for dataset in train_datasets:
        dataloader = build_train_dataloader(dataset, cfg)
        train_dataloaders.append(dataloader)
        train_size += len(dataloader)

    print('train size: ', train_size)

    val_dataloader = build_val_dataloader(val_dataset, cfg)
    ...
    in_channels = train_datasets[0].datasets[0].in_channels
    out_channels = train_datasets[0].datasets[0].out_channels

    # domain_rows and domain_cols are used to determine the number of modes
    # used in fourier models.
    _, domain_rows, domain_cols = train_datasets[0].datum_dim()
    ...
    total_iters = exp.train.max_epochs * train_size
    ...
    if exp.lr_scheduler.name == 'step':
        warm_schedule = torch.optim.lr_scheduler.StepLR(optimizer,
                                                        ...
                                                        step_size=exp.lr_scheduler.patience * train_size,
        ...
        warm_schedule = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
                                                                   T_max=exp.train.max_epochs * train_size,
